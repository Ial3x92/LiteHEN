...
│   ├── service_facade.cpp       # 进程内 FTP / ShadowMount+ 生命周期
│   ├── shadowmount_main.cpp     # ShadowMount+ 源码模块适配器（启动/停止序列）
...
│   ├── util_platform.h
│   └── cheats/                  # 金手指公共/内部头
└── CMakeLists.txt                # util.elf 与 ftpsrv / ShadowMountPlus 源码模块构建
```
...
| ShadowMount+ | `service_facade.cpp` + `shadowmount_main.cpp` + `third_party/ShadowMountPlus` + `third_party/sqlite` | main / IPC |
...
ipc_handle ──► CheatService / CheatSyncService / FtpServiceFacade / ShadowMountServiceFacade
...
| ShadowMount+ worker | `ShadowMountServiceFacade::start` | 配置启用期间 | 运行 `shadowmount_module_main`（扫描/挂载循环） |
...
| `TOGGLE_SHADOWMOUNT` | 启停进程内 ShadowMount+ 模块 | `ShadowMountServiceFacade` |
...
| ShadowMountPlus + sqlite | 编译进 util 的游戏扫描/挂载模块（`1.6beta16`）及其 app 数据库 amalgamation |
