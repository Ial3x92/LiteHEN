...
USER builder
CMD ["/bin/bash", "-lc", "set -euo pipefail; rm -rf /tmp/Lite_HEN-build; cp -a /workspace/. /tmp/Lite_HEN-build; python3 -c \"from pathlib import Path; files=list(Path('/tmp/Lite_HEN-build').rglob('*.sh'))+list(Path('/tmp/Lite_HEN-build').rglob('*.bash')); [p.write_bytes(p.read_bytes().replace(bytes([13]), bytes())) for p in files]\"; git -C /tmp/Lite_HEN-build submodule update --init --recursive; cd /tmp/Lite_HEN-build; export PS5_PAYLOAD_SDK=/opt/ps5-payload-sdk; export PATH=/usr/lib/llvm-18/bin:${PS5_PAYLOAD_SDK}/bin:${PATH}; export LLVM_CONFIG=/usr/lib/llvm-18/bin/llvm-config; ./scripts/build.sh --jobs 8; rm -rf /workspace/build; cp -a /tmp/Lite_HEN-build/build /workspace/build"]
