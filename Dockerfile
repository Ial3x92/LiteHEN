...
USER builder
CMD ["/bin/bash", "-lc", "set -euo pipefail; rm -rf /tmp/lite-hen-build; cp -a /workspace/. /tmp/lite-hen-build; python3 -c \"from pathlib import Path; files=list(Path('/tmp/lite-hen-build').rglob('*.sh'))+list(Path('/tmp/lite-hen-build').rglob('*.bash')); [p.write_bytes(p.read_bytes().replace(bytes([13]), bytes())) for p in files]\"; git -C /tmp/lite-hen-build submodule update --init --recursive; cd /tmp/lite-hen-build; export PS5_PAYLOAD_SDK=/opt/ps5-payload-sdk; export PATH=/usr/lib/llvm-18/bin:${PS5_PAYLOAD_SDK}/bin:${PATH}; export LLVM_CONFIG=/usr/lib/llvm-18/bin/llvm-config; ./scripts/build.sh --jobs 8; rm -rf /workspace/build; cp -a /tmp/lite-hen-build/build /workspace/build"]
