...
static int test_display_strip_user(void) {
  std::string a = display_path_for_ui("/user/data/lite-HEN/payloads/x.elf");
  std::string b = display_path_for_ui("/user");
  TEST_ASSERT_STREQ("/data/lite-HEN/payloads/x.elf", a.c_str());
  TEST_ASSERT_STREQ("", b.c_str());
  return 0;
}

static int test_display_map_usb(void) {
  std::string a = display_path_for_ui("/usb0/lite-hen/payloads/a.elf");
  std::string b = display_path_for_ui("/usb3/x");
  TEST_ASSERT_STREQ("/mnt/usb0/lite-hen/payloads/a.elf", a.c_str());
  TEST_ASSERT_STREQ("/mnt/usb3/x", b.c_str());
  return 0;
}

static int test_display_passthrough(void) {
  std::string a = display_path_for_ui("/data/lite-HEN/x");
  ...
  TEST_ASSERT_STREQ("/data/lite-HEN/x", a.c_str());
...
static int test_elf_key_from_name(void) {
  ...
  TEST_ASSERT_TRUE(
      elf_key_from_name("/user/data/lite-HEN/payloads/x.elf", key, sizeof(key)));
