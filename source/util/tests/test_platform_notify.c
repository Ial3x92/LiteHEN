...
static int test_notify_format_prefix(void) {
  ...
  TEST_ASSERT_STREQ("[Lite_HEN] hello world", out);
  return 0;
}
...
static int test_notify_format_truncates(void) {
  ...
  TEST_ASSERT_TRUE(strncmp(out, "[Lite_HEN]", 10) == 0);
  return 0;
}
...
static int test_notify_format_localized(void) {
  ...
  TEST_ASSERT_STREQ("[Lite_HEN] 无法提升权限", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_EN);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] Unable to raise privileges", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_AR);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] تعذّر رفع الامتيازات", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_ZH_HANT);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] 無法提升權限", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_JA);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] 権限を昇格できません", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_FR);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] Impossible d’élever les privilèges", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_DE);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] Berechtigungen konnten nicht erhöht werden",
                    out);
  onion_notify_set_language(ONION_NOTIFY_LANG_KO);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] 권한을 올릴 수 없습니다", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_ES);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] No se pudieron elevar los privilegios", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_PT_BR);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] Não foi possível elevar os privilégios", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_IT);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] Impossibile elevare i privilegi", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_RU);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] Не удалось повысить привилегии", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_PL);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] Nie udało się podnieść uprawnień", out);
  onion_notify_set_language(ONION_NOTIFY_LANG_TH);
  format_msg(out, sizeof(out), 1, "notify.priv.unable");
  TEST_ASSERT_STREQ("[Lite_HEN] ไม่สามารถยกระดับสิทธิ์ได้", out);
  return 0;
}
...
static int test_notify_payload_localized(void) {
  ...
  TEST_ASSERT_STREQ("[Lite_HEN] 正在加载 Payload demo.elf...", out);

  format_msg(out, sizeof(out), 1,
             "notify.payload.launched", "/data/demo.elf",
             "demo");
  TEST_ASSERT_STREQ(
      "[Lite_HEN] Payload 已启动\n路径：/data/demo.elf\n标识：demo", out);
...
static int test_notify_rich_localizes_both_text_fields(void) {
  ...
  onion_notify_rich("notify.brand", "notify.boot.starting", "/icon.png",
                    "download", "43");
  TEST_ASSERT_TRUE(strstr(g_rich_payload, "Lite_HEN 正在启动...") != NULL);
  onion_notify_set_language(ONION_NOTIFY_LANG_EN);
  return 0;
}
