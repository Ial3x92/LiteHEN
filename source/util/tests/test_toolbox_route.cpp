...
static int test_plugins_registry(void) {
  ...
  TEST_ASSERT_TRUE(find_by_toggle_id("id_plugin_shadowmount") != nullptr);
  ...
  by_res = find_by_config_xml_resource(cfg_res("shadowmount.xml"));
  TEST_ASSERT_TRUE(by_res != nullptr);
  TEST_ASSERT_STREQ("shadowmount", by_res->key);
