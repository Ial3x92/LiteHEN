...
static int test_en(void) {
  ...
  TEST_ASSERT_TRUE(std::strcmp(tr("ftp.autoload"), "Start FTP with ") == 0);
  ...
  TEST_ASSERT_TRUE(std::strcmp(tr("startup.open_after_load"),
                               "Automatically open after  loads") == 0);
  ...
  TEST_ASSERT_TRUE(std::strcmp(tr("pkg.msg.installing"),
                               " is installing the selected PKG") == 0);
...
static int test_ar(void) {
  set_lang(Lang::Ar);
  TEST_ASSERT_TRUE(std::strcmp(tr("root.title"), "★صندوق أدوات ") == 0);
...
static int test_fr(void) {
  set_lang(Lang::Fr);
  TEST_ASSERT_TRUE(std::strcmp(tr("root.title"), "★Boîte à outils ") ==
                   0);
...
static int test_es(void) {
  set_lang(Lang::Es);
  TEST_ASSERT_TRUE(
      std::strcmp(tr("root.title"), "★Caja de herramientas ") == 0);
...
static int test_pt_br(void) {
  set_lang(Lang::PtBr);
  TEST_ASSERT_TRUE(
      std::strcmp(tr("root.title"), "★Caixa de ferramentas ") == 0);
  TEST_ASSERT_TRUE(std::strcmp(tr("cheats.link"), "Cheats") == 0);
...
static int test_it(void) {
  set_lang(Lang::It);
  TEST_ASSERT_TRUE(
      std::strcmp(tr("root.title"), "★Cassetta degli attrezzi ") == 0);
...
static int test_ru(void) {
  set_lang(Lang::Ru);
  TEST_ASSERT_TRUE(std::strcmp(tr("root.title"), "★Инструменты ") == 0);
...
static int test_pl(void) {
  set_lang(Lang::Pl);
  TEST_ASSERT_TRUE(std::strcmp(tr("root.title"), "★Narzędzia ") == 0);
