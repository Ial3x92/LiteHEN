 ...
 * concerns to the host util process: signals are replaced by
 * shadowmount_module_request_stop(), the single-instance scan belongs to the
 * service facade, and the SceUserService lifetime is owned by util's main().
 */
...
#endif
#ifndef Lite_HEN_SHADOWMOUNT_TEMPLATE
#define Lite_HEN_SHADOWMOUNT_TEMPLATE "config.ini.example"
#endif
#ifndef Lite_HEN_SHADOWMOUNT_ICON
#define Lite_HEN_SHADOWMOUNT_ICON "smp_icon.png"
#endif

/* Upstream generates this asset with xxd at Makefile time; embed the pinned
 * notification icon directly so rich notifications keep their artwork. */
...
__asm__(".section .rodata\n"
        ...
        ".align 16\n"
        "smp_icon_png:\n"
        ".incbin \"" Lite_HEN_SHADOWMOUNT_ICON "\"\n"
        "smp_icon_png_end:\n"
...
/* Upstream references this kernel NID directly (see sm_mdbg.c). Older
 * libkernel_sys stubs do not carry it, so a weak alias keeps the link
 * working; at load time the real kernel export overrides a weak definition.
 * The fallback of 0 only disables the optional mdbg crash-log monitor. */
 ...
 * sm_*.c modules call these entry points; upstream defined them next to
...
/* Upstream generates this asset with xxd at Makefile time; embed the pinned
 * config template directly so first run can create the default file. */
...
__asm__(".section .rodata\n"
        ...
        ".incbin \"" Lite_HEN_SHADOWMOUNT_TEMPLATE "\"\n"
 ...
 * scanner starts. Reuse Lite_HEN's sysctl-based lookup instead of upstream's
...
/* Startup steps that lead into the blocking scanner loop; returns after the
 * loop finishes or when startup must skip straight to shutdown. */
...
void shadowmount_module_request_stop() {
  request_shutdown_stop("Lite_HEN facade");
}
