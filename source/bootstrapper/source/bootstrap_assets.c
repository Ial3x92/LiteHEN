...
bool bootstrap_assets_write(void) {
  ...
  mkdir("/data/Lite_HEN/assets", 0777);

  const bool startup_icon_ready = write_blob_file(
      "/data/Lite_HEN/Lite_HEN.png", sicon_start, sicon_size);

  for (size_t i = 0; i < sizeof(kEmbeddedIcons) / sizeof(kEmbeddedIcons[0]);
       ++i) {
    char path[256];
    snprintf(path, sizeof(path), "/data/Lite_HEN/assets/%s.png",
  ...
  (void)write_blob_file("/system_ex/vsh_asset/Lite_HEN.png", sicon_start,
                        sicon_size);
