       ...
       [](ps5ui::Group& rg) {
         ...
         if (remote_play_alert) {
           ...
           rg.link("id_remote_play", toolbox_i18n::tr("remote_play.title"),
                  "remote_play.xml", std::nullopt); 
         }
       },
       std::nullopt, std::nullopt, "id_remote_play") 

      // 4. Integrazione Attivazione Account Offline
      .link("id_account_activation", toolbox_i18n::tr("account.link"),
            "account.xml", std::nullopt) 

    // 5. Configurazione Scorciatoie e Ambiente NP
      .list("id_cheats_shortcut", toolbox_i18n::tr("sc.cheats"),
            [](ps5ui::ListBuilder& L) {
              // Caching statico delle stringhe per azzerare i cicli di strcmp ad ogni ridisegno
              static const std::string t_off        = toolbox_i18n::tr("sc.off");
              static const std::string t_r3_l3      = toolbox_i18n::tr("sc.r3_l3");
              static const std::string t_l2_tri     = toolbox_i18n::tr("sc.l2_tri");
              static const std::string t_long_opt   = toolbox_i18n::tr("sc.long_options");
              static const std::string t_long_share = toolbox_i18n::tr("sc.long_share");
              static const std::string t_share      = toolbox_i18n::tr("sc.share");

              L.item("id_cheats_shortcut_0", t_off.c_str(),        "0")
               .item("id_cheats_shortcut_1", t_r3_l3.c_str(),      "1")
               .item("id_cheats_shortcut_2", t_l2_tri.c_str(),     "2")
               .item("id_cheats_shortcut_3", t_long_opt.c_str(),   "3")
               .item("id_cheats_shortcut_4", t_long_share.c_str(), "4")
               .item("id_cheats_shortcut_5", t_share.c_str(),      "5");
            },
