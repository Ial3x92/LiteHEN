...
//
// Paths:
//   primary  /data/Lite_HEN/config.ini          (elevated daemons)
//   shellui  /user/data/Lite_HEN/config.ini     (SceShellUI sandbox view)
...
inline constexpr const char *kConfigPathPrimary = "/data/Lite_HEN/config.ini";
inline constexpr const char *kConfigPathShellui = "/user/data/Lite_HEN/config.ini";
...
struct Settings {
  // [startup]
  // Page to open after Lite_HEN finishes loading.
  ...
  // [kstuff]
  // Load the embedded/override kstuff payload when Lite_HEN starts.
  bool kstuff_autoload = false;

  // [ftp]
  // Start the built-in FTP server the next time Lite_HEN launches.
  bool ftp_autoload = false;
  // TCP listen port for the built-in FTP server (1..65535).
  int ftp_port = kFtpPortDefault;

  // [shadowmount]
  // Start the built-in ShadowMount+ module the next time Lite_HEN launches.
